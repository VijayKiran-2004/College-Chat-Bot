...
from app.services.logger_service import ResponseLogger  # noqa: E402
...
def initialize_systems():
    ...
    try:
        ...
        print(
            f"✓ Response Logger initialized. Saving logs to: {response_logger.log_file}"
        )
...
def log_async(
    user_query, bot_response, time_taken, session_id, source, context_snippets=None
):
...
async def chat(request: QueryRequest, background_tasks: BackgroundTasks):
    ...
    if query_router is None:
        raise HTTPException(
            status_code=503,
            detail=(
                "System not initialized. Please check the server logs "
    ...
    try:
        ...
        session_history[session_id].append({"role": "user", "message": request.message})

        # Prepare chat history context (last 3 turns)
        recent_history = (
            session_history[session_id][-6:]
        ...
        if use_streaming:
            ...
            try:
                ...
                with ThreadPoolExecutor(max_workers=1) as executor:
                    future = executor.submit(
                        query_router.route_query, request.message, recent_history
                    )
                    try:
                        ...
                        session_timeout_count[session_id] = (
                            session_timeout_count.get(session_id, 0) + 1
                        )

                        if session_timeout_count[session_id] == 1:
                            answer = (
                                "I'm taking a bit too long to think about that."
                                "Could you try asking in a simpler way?"
                            )
                        else:
                            answer = (
                                "I apologize, but I'm having trouble "
                                "processing this question right now."
                                "Please try a different question or "
                                "contact the college office for immediate assistance."
                            )
                            session_timeout_count[session_id] = 0
            ...
            session_history[session_id].append({"role": "bot", "message": answer})
            ...
            return QueryResponse(
                ...
                confidence=(
                    100
                    if response_accuracy == "High"
                    else 50 if response_accuracy == "N/A" else 0
                ),
...
if __name__ == "__main__":
    ...
    # Run with uvicorn
    uvicorn.run(
        "backend:app", host="127.0.0.1", port=8000, reload=False, log_level="info"
    )
