...
app.add_middleware(
    CORSMiddleware,
    # Allow all origins for local development and file:// access
    allow_origins=["*"],
...
async def chat(request: QueryRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        recent_history = session_history[session_id][-6:] if len(
            session_history[session_id]) > 0 else []
                ...
                try:
                    ...
                    for chunk in rag_system(request.message, stream=True):
                        ...
                        if isinstance(chunk, dict) and chunk.get(
                            ...
                            # Forward metadata to client so test scripts can
                            # see it
                    ...
                    # Log the COMPLETE response after streaming finishes in
                    # background
                    if response_logger:
            ...
            try:
                ...
                with ThreadPoolExecutor(max_workers=1) as executor:
                    ...
                    try:
                        ...
                        session_timeout_count[session_id] = session_timeout_count.get(
                            session_id, 0) + 1
