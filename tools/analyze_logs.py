...
def main():
    ...
    if TIME_COL in df.columns:
        ...
        print(
            f"  Fast     (< 1 s)  : {len(fast):4d}  ({len(fast)/n*100:.1f}%)")
        print(
            f"  Moderate (1–10 s) : {len(moderate):4d}  ({len(moderate)/n*100:.1f}%)")
        print(
            f"  Slow     (10–50s) : {len(slow):4d}  ({len(slow)/n*100:.1f}%)")
        print(
