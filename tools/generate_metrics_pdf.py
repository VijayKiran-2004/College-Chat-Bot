...
ST = {
    "cover_main": ps(
        "cm", fontSize=32, leading=40, fontName="Helvetica-Bold", textColor=C["white"]
    ),
    "metric_h": ps(
        "mh", fontSize=13, leading=18, fontName="Helvetica-Bold", textColor=C["navy"]
    ),
    "col_lbl": ps(
        "cl",
        fontSize=8,
    ...
    "formula_h": ps(
        "fh", fontSize=8, leading=10, fontName="Helvetica-Bold", textColor=C["white"]
    ),
    "formula_b": ps(
        "fb",
        fontSize=9.5,
    ...
    "tbl_hdr": ps(
        "th", fontSize=9, leading=12, fontName="Helvetica-Bold", textColor=C["white"]
    ),
    "tbl_cell": ps(
        "tc", fontSize=9, leading=13, fontName="Helvetica", textColor=C["grey_m"]
    ),
    "sec_label": ps(
        "sl", fontSize=9, leading=12, fontName="Helvetica-Bold", textColor=C["accent"]
    ),
    "sec_title": ps(
        "st", fontSize=18, leading=24, fontName="Helvetica-Bold", textColor=C["white"]
    ),
    "overview_h": ps(
        "oh",
        fontSize=11,
...
def section_divider(number, title, subtitle=""):
    rows = [
        [Paragraph(f"SECTION {number}", ST["sec_label"]), ""],
...
def score_bands(bands):
    rows = [
        [Paragraph("SCORE", ST["col_lbl"]), Paragraph("INTERPRETATION", ST["col_lbl"])]
    ]
    for r, m, ck in bands:
        rows.append(
            [
                Paragraph(f"<b>{r}</b>", ps("rb", fontSize=9, textColor=C[ck])),
                Paragraph(m, ST["tbl_cell"]),
            ]
        )
...
def metric_card(
    title, sheet_tag, accent_color, what, formula_lines, how, bands, note=None
):
    hdr = Table(
        [
            [
                ...
                Paragraph(
                    sheet_tag,
                    ps("sh", fontSize=8, textColor=colors.white, alignment=TA_RIGHT),
                ),
            ]
        ],
    ...
    what_t = Table(
        [[Paragraph("WHAT IT MEASURES", ST["col_lbl"])], [Paragraph(what, ST["body"])]],
        colWidths=[PAGE_W],
    )
    what_t.setStyle(
        TableStyle(
    ...
    how_t = Table(
        [[Paragraph("HOW IT WORKS", ST["col_lbl"])], [Paragraph(how, ST["body"])]],
        colWidths=[PAGE_W],
    )
    how_t.setStyle(
        TableStyle(
    ...
    note_t = (
        [Table([[Paragraph(f"ℹ {note}", ST["note"])]], colWidths=[PAGE_W])]
        if note
        else []
    )
    if note_t:
...
def draw_cover(canv, doc):
    ...
    canv.rect(0, 0, W, H, fill=1)
    canv.setFillColor(C["accent"])
    canv.rect(0, 0, 0.5 * cm, H, fill=1)
    canv.setFont("Helvetica-Bold", 38)
    canv.setFillColor(C["white"])
    canv.drawString(2.2 * cm, H * 0.72, "Objective")
    canv.drawString(2.2 * cm, H * 0.64, "Metrics")
    canv.setFillColor(C["accent"])
    canv.drawString(2.2 * cm, H * 0.56, "Reference")
    canv.setFont("Helvetica", 13)
    canv.setFillColor(colors.HexColor("#90CAF9"))
    canv.drawString(
        2.2 * cm, H * 0.49, "Simplified 'No-Judge' evaluation for the College Chatbot"
    )
    canv.restoreState()
...
def on_page(canv, doc):
    if doc.page == 1:
        ...
        canv.drawString(
            2 * cm, H - 1.3 * cm, "College Chatbot · Objective Metrics Guide"
        )
        canv.drawRightString(W - 2 * cm, H - 1.3 * cm, f"Page {doc.page}")
        canv.restoreState()
...
story.append(
    Paragraph(
        "Following user feedback, the qualitative LLM Judge has been removed. "
        "Evaluation is now 100% objective, "
        "focusing on semantic truth and routing accuracy.",
        ST["body"],
    )
)
...
# Glance table
ov = [
    [Paragraph(h, ST["tbl_hdr"]) for h in ["Metric", "Sheet", "Weight", "Goal"]],
...
story += metric_card(
    ...
    [("0-2s", "Great", "green"), (">5s", "Slow", "red")],
    "Background logging ensures zero impact on user.",
)
story += metric_card(
    "Context",
    "BOTH",
    C["accent"],
    "Raw data retrieved to answer the question.",
    ["KB Facts | SQL Rows | PDF Snippets"],
    "Passed from backend to log and test suite.",
    [("Present", "Transparent", "teal"), ("Empty", "Review", "amber")],
    "This is the primary transparency tool.",
)
...
story += metric_card(
    ...
    ["F1 (BERT-style) mapping"],
    "Uses all-MiniLM-L6-v2 sentence embeddings.",
    [("80-100%", "Accurate", "green"), ("<50%", "Suspect", "red")],
    "Weighted at 50% of Final Accuracy.",
)
story += metric_card(
    "Link Validity",
    "EVAL",
    C["amber"],
    "Whether links in the answer are live.",
    ["X/Y Valid"],
    "Pings URLs via HTTP HEAD/GET.",
    [("All Valid", "Good", "green"), ("Partial", "Stale", "red")],
    "Weighted at 25% of Final Accuracy.",
)
...
story += metric_card(
    ...
    "EVAL",
    C["blue"],
    "Total balanced quality score.",
    ["BERT(0.5) + Source(0.25) + Links(0.25)"],
    "Weighted average of all objective checks.",
    [("85-100%", "Reliable", "green"), ("<65%", "Fix Needed", "red")],
    "Strictly objective; no LLM judge bias.",
)
