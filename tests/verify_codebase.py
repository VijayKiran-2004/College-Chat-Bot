    ...
    try:
        ...
        if len(lines) >= 3:
            ...
            if free_gb < 1.0:
                ...
                print("[NOTE] Free memory is tight. Close heavy apps like Chrome for best performance.")
            else:
    ...
    for name, rel_path in files_to_check:
        ...
        if full_path.exists():
            ...
            if "corpus" in rel_path:
                print("         Run 'python scripts/tkrcet_scraper.py', then 'generate_vectors.py', 'corpus_converter.py', and 'ingest.py'")
            elif "students.db" in rel_path:
                print("         The students.db file is missing. Please check if you have a backup or need to regenerate it.")
    print()
...
def check_env():
    ...
    if env_path.exists():
        ...
        print("[FAILED] .env file NOT found. Run 'copy .env.example .env' and add your key.")
    print()
