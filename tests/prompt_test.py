...
from app.services.logger_service import EVAL_COLUMNS, EVAL_SHEET  # noqa:E402
...
def bertscore_f1(answer, context):
    """Compute a BERTScore-style F1 using sentence-transformers.
...
def source_appropriateness(query, source):
    ...
    is_sql = any(
        kw in q
        for kw in ["how many", "list all", "who placed", "roll no", "cgpa", "student"]
    )
    if is_sql and source == "SQL Database":
        return 1.0
    if not is_sql and source in ["Knowledge Base", "RAG"]:
        return 1.0
...
for prompt in test_prompts:
    ...
    accuracy_pct = round(
        (bert_f1 * 0.50 + src_score * 0.25 + link_score * 0.25) * 100, 2
    )
...
if results:
    ...
    print(
        f"\n📊 Summary — Avg Accuracy: {df['Accuracy %'].mean():.1f}% |"
        f" Avg Latency: {df['Latency (s)'].mean():.2f}s"
    )
else:
    print("\n⚠ No results.")
