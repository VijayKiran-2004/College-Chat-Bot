    ...
    with open(output_path, "w", encoding="utf-8") as f:
        for idx, chunk in enumerate(chunks):
            ...
            doc = {
                ...
                "metadata": {"source": source, "url": url, "original_index": idx},
            }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input",
        default=str(_PROJECT_ROOT / "data" / "chunks" / "unified_vectors.json"),
        help="Input JSON file path",
    )
    parser.add_argument(
        "--output",
        default=str(_PROJECT_ROOT / "data" / "chunks" / "corpus_ultrarag.jsonl"),
        help="Output JSONL file path",
    )
    parser.add_argument(
        "--verify", action="store_true", help="Verify the output file after conversion"
    )
