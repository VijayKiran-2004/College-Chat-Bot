...
SCRAPED_JSONL = str(
    _PROJECT_ROOT / "app" / "database" / "vectordb" / "scraped_data.jsonl"
)
...
def clean_text(text):
    ...
    text = re.sub(
        r"(Skip to content|Menu|Search|Copyright ©.*|All Rights Reserved.*)",
        "",
        text,
        flags=re.IGNORECASE,
    )
...
def load_scraped_data():
    ...
    if os.path.exists(SCRAPED_JSONL):
        print("Loading scraped data from JSONL...")
        ...
        print("Loading scraped data directly from Master JSON...")
        try:
...
def main():
            ...
            unified_vectors.append(
                {
                    ...
                    "source": source,
                    "url": url,
                    "metadata": {"title": title, "source": source, "url": url},
                }
            )
    ...
    print("Next step: python scripts/corpus_converter.py")
    print("\n" + "=" * 70)
