...
def main():
    ...
    try:
        ...
        **Task**: Rewrite this content, preserving ALL factual details (dates, names, fees, rules, phone numbers, addresses) but removing navigation menus, copyright footers, ads, and contact forms.
        ...
        print(
            f"⚠ Warning: Could not initialize LLM ({e}). Proceeding with raw text.")
        llm = None
    ...
    for idx, doc in enumerate(documents):
        ...
        if llm:
            try:
                print(
                    f"  [{idx+1}/{len(documents)}] Cleaning: {title[:30]}...",
                    end='',
                    flush=True)
    ...
    if ids:
        for i in range(0, len(ids), BATCH_SIZE):
            ...
            print(
                f"  - Processed batch {i//BATCH_SIZE + 1}/{(len(ids)-1)//BATCH_SIZE + 1}")
    else:
