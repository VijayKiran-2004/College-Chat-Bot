...
class QueryRouter:
    def __init__(self):
        ...
        self.reasoning_chain = DeepReasoningChain(
            rag_system=self.rag_system, sql_system=self.sql_system
        )
    ...
    def route_query(self, query, chat_history=None):
        ...
        # Also force 'general' if the query starts with 'who is'
        # without student identifiers
        query_lower = query.lower().strip()
        student_ids = ["placed", "topper", "highest cgpa", "highest package", "roll"]
        if query_lower.startswith("who is") and not any(
            s in query_lower for s in student_ids
        ):
            intent = "general"
