        ...
        with self.lock:
            ...
            if needs_reset:
                if self.log_file.exists():
                    ...
                    backup = self.log_file.with_name(
                        f"response_log_STALE_{timestamp}.xlsx"
                    )
                    print(
                        "⚠ Self-healing: Excel columns outdated. "
                        f"Archiving to {backup.name}"
                    )
                    try:
                ...
                print(
                    "✓ Self-healing: Fresh log created with unified schema "
                    f"at {self.log_file}"
                )
    ...
    def log_response(
        ...
        with self.lock:
            try:
                ...
                print("⚠ ERROR: Could not write to Excel. Is it open?")
            except Exception as e:
