...
class SQLSystem:
    ...
    def extract_entities(self, query):
        ...
        roll_match = re.search(r"\b(\d{2}[A-Z]\d{2}[A-Z]\d{4})\b", query) or re.search(
            r"\b(\d{5,10})\b", query
        )
        if roll_match:
        ...
        # Extract Gender
        if re.search(
            r"\b(girl|girls|female|females|woman|women|ladies)\b", query_lower
        ):
    ...
    def build_sql_query(self, entities):
        ...
        if "placed" in entities:
            if entities["placed"]:
                conditions.append(
                    '"COMPANY PLACED" IS NOT NULL AND'
                    "\"COMPANY PLACED\" != 'Not Placed'"
                )
            else:
                conditions.append(
                    '("COMPANY PLACED" IS NULL OR "COMPANY PLACED" = \'Not Placed\')'
                )
    ...
    def _get_fallback_links(self, query: str) -> str:
        ...
        q = query.lower()
        if any(
            k in q for k in ["placed", "placement", "package", "company", "recruit"]
        ):
            return (
                "\n\n⚠ I couldn't query the specific data right now."
        ...
        return (
            "\n\n⚠ I couldn't query the database right now. Visit "
            "[TKRCET](https://tkrcet.ac.in/) for more info."
        )
    ...
    def _generate_sql(self, query):
        ...
        hints = (
            "GENDER values are 'Male' and 'Female'. 'COMPANY PLACED' contains "
            "company name or 'Not Placed'. To find placed students, "
            "check \"COMPANY PLACED\" != 'Not Placed'.",
        )

        prompt = f"""You are a SQL expert. Convert the user's natural language
            query into a valid SQLite query for the 'students' table.
...
3. User: "how many students are placed?"
   SQL: SELECT COUNT(*) FROM students WHERE "COMPANY PLACED"
   IS NOT NULL AND "COMPANY PLACED" != 'Not Placed'

Query: {query}

Rules:
1. Return ONLY the raw SQL query. No markdown, no explanations.
2. ALWAYS quote all column names using double quotes
    (e.g., "PASSED YEAR" not PASSED_YEAR).
...
SQL:"""
    ...
    def query_students(self, query, chat_history=None):
        ...
        if chat_history and len(chat_history) > 0:
            ...
            if (
                is_follow_up and len(query.split()) < 6
                ...
                for turn in reversed(chat_history[-4:]):  # Check last 4 turns
                    if turn.get("role") == "user":
                        ...
                        if any(
                            kw in prev_query
                            for kw in ["placed", "placement", "company", "students"]
                        ):
                            query = f"{prev_query} {query}"  # Merge context
                            query_lower = query.lower()
                            print(
                                f"  [SQL Context] Merged with previous query: {query}"
                            )
                            break
        ...
        if year_match and any(
            kw in query_lower
            for kw in ["placed", "placement", "got job", "recruited", "how many"]
        ):
            year = int(year_match.group(1))
            try:
                df_total = pd.read_sql_query(
                    f"SELECT COUNT(*) as total FROM students"
                    f" WHERE `PASSED YEAR` = {year}",
                    self.conn,
                )
            ...
            try:
                df = pd.read_sql_query(
                    """SELECT NAME, BRANCH, "COMPANY PLACED", "PACKAGE"
                       ...
                       AND "PACKAGE" != '0'
                       ORDER BY CAST(REPLACE(REPLACE("PACKAGE", 'LPA', ''), ' ', '')
                       AS REAL) DESC
                       LIMIT 1""",
                    self.conn,
                )
                ...
                return (
                    ...
                    f"\n_(Individual data shared with permission."
                    " Contact admin for verification.)_"
                )
            except Exception:
                # Fallback: just return count of placed students with any package info
                return (
                    "Package details are not available in structured form."
        ...
        company_match = re.search(
            r"(?:placed in|got placed in|students? in|placed at)\s"
            r"+(.+?)(?:\?|$|in \d{4})",
            query_lower,
        )
        ...
        if company_match and branch_in_query:
            ...
            try:
                ...
                if count == 0:
                    return (
                        f"No {branch_in_query} students found placed in a company"
                        f" '{company_name}'."
                    )
                return (
                    f"• **{count} {branch_in_query} student"
                    f"{'s' if count != 1 else ''}**"
                    f" placed in **{company_name.title()}**\n"
                    f"\n- [Placement Cell](https://tkrcet.ac.in/placement/)"
                )
            except Exception as e:
        ...
        if not entities:
            ...
            if not sql_query:
                return (
                    "I couldn't understand the student query."
                    " Please specify student ID, name, department, or conditions."
                    + self._get_fallback_links(query)
                )
            ...
            if not is_safe:
                ...
                return (
                    "I couldn't safely process that query."
                    " Please try rephrasing with specific student details."
                )
        ...
        try:
            ...
            if len(result_df) == 1 and "roll_no" in entities:
                ...
                if "COMPANY PLACED" in result_df.columns:
                    placed = result_df[
                        result_df["COMPANY PLACED"].notna()
                        & (result_df["COMPANY PLACED"] != "Not Placed")
                    ]
                    ...
                    if len(placed) > 0:
                        ...
                        response += (
                            f"• Top recruiters: {', '.join(companies.index.tolist())}\n"
                        )
                ...
                if "BRANCH" in result_df.columns:
                    ...
                    response += (
                        f"• Branch-wise:"
                        f" {', '.join([f'{k}: {v}' for k, v in branches.items()])}\n"
                    )

                response += (
                    "\n(Individual student data is protected. "
                    "Please contact administration for specific records.)"
                )
                ...
                if (
                    "placed" in query.lower()
                    or "package" in query.lower()
                    or "company" in query.lower()
                ):
                    links += "\n- [Placement Cell](https://tkrcet.ac.in/placement/)"
                    has_links = True
                ...
                if (
                    "exam" in query.lower()
                    or "result" in query.lower()
                    or "passed" in query.lower()
                    or "failed" in query.lower()
                ):
                    links += (
                        "\n- [Exam Results Portal]"
                        "(https://tkrcet.ac.in/exam-branch/results/)"
                    )
                    has_links = True
